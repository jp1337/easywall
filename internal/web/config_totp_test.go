...
func TestConfig_DisablingClearsBothKeys(t *testing.T) {
	dir := t.TempDir()
	path := dir + "/web.toml"
	// The comment is load-bearing. Without one in the fixture this test passes
	// identically whether mergeConfig edited the file in place or gave up and
	// re-encoded it from the struct — and the fallback is exactly the silent
	// failure the merge exists to avoid. With one, a fallback is visible.
	_ = os.WriteFile(path, []byte(`# A comment that has to survive the factor being switched off.
session_key    = "test-session-key-32bytes-padding!"
username       = "admin"
password       = ""
totp_secret    = "JBSWY3DPEHPK3PXP"
...
`), 0600)
	...
	if !strings.Contains(string(raw), "A comment that has to survive") {
		t.Errorf("switching the factor off took the file's comments with it, so the "+
			"merge fell back to the encoder\n--- file ---\n%s", raw)
	}
