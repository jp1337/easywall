...
func TestDescribeRuleChange_CountsStructuredRules(t *testing.T) {
	before := Rules{TCP: []PortRule{{Port: "22"}}}
	after := Rules{TCP: []PortRule{{Port: "22"}, {Port: "443"}}}
...
func TestDescribeRuleChange_EditInPlaceIsNotSilent(t *testing.T) {
	before := Rules{TCP: []PortRule{{Port: "22", Description: "ssh"}}}
	after := Rules{TCP: []PortRule{{Port: "22", Description: "SSH access"}}}
...
func TestDescribeStructChange_NamesTheSwitchThatMoved(t *testing.T) {
	before := FirewallOptions{SYNFlood: true, PortScan: true}
	after := FirewallOptions{SYNFlood: true, PortScan: false, TCPRSTFlood: true}
...
func TestDescribeStructChange_RecursesIntoNestedSettings(t *testing.T) {
	before := NetworkSettings{Docker: DockerConfig{Enabled: false}}
	after := NetworkSettings{Docker: DockerConfig{Enabled: true}}
...
func TestDescribeStructChange_SaysSoWhenNothingMoved(t *testing.T) {
	opts := FirewallOptions{SYNFlood: true}
