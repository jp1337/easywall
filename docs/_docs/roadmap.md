...
```
Understand what you do   2.12  The configuration comes from outside, and the page says so
...
|---|---|---|
| **2.12** | **The configuration comes from outside, and the page says so** — an environment variable can set any of thirteen keys, the interface shows which values came from it, and the doubled save button goes | Every setting was already overridable from the environment and nothing said so, so a value changed in the interface silently reverted on the next restart. The precedence inverts: what the operator stored wins, and there is a way back to the environment's value |
| **2.13** | **Behind a proxy, easywall knows who you are** — a *list* of trusted proxy addresses, and the resolved client becomes the truth in the audit log, the reachability verdict and the login rate limiter | Behind a reverse proxy every request looks like the proxy, so the limiter's five attempts per ten minutes are shared by everyone and one attacker exhausts them for all. A list, never a boolean: a boolean that trusts the header is the vulnerability three advisories describe |
