...
| **2.31** | **Eight languages** — Spanish, Portuguese (BR), Italian, Dutch, Polish, Russian, Chinese (Simplified), Japanese | One pass, once the string set is stable. No RTL: that is a design-system change, not a translation. `fr` sits at 472 of 589 keys (80.1%) while `en` and `de` hold exact parity; finishing it is part of this pass, not a separate errand |
