...
| An opt-in count of installations | sends one request a day to `telemetry.wdkro.de` — a random identifier and the version, nothing else — and stays off until switched on. Withdrawable from the System page without needing the core reachable. Detail: [Security]({{ '/security/' | relative_url }}#every-request-that-goes-out) |
