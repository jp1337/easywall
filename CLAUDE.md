...
| | |
|---|---|
| `cmd/easywall-core`, `cmd/easywall-web` | the two entry points; both take `-config`, `-write-config` and `-version`. `easywall-web` also takes `-healthcheck`, the container's HEALTHCHECK; `easywall-core` also has five subcommands — `status`, `health`, `selftest`, `panic`, `resume` |
