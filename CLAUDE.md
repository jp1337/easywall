...
| | |
|---|---|
| `cmd/easywall-core`, `cmd/easywall-web` | the two entry points; both take `-config`, `-write-config` and `-version`. `easywall-web` takes nothing else; `easywall-core` also has five subcommands — `status`, `health`, `selftest`, `panic`, `resume` |
...
| [protocol](docs-tech/protocol.md) | the socket protocol: twenty-two commands, and the one field that is not typed |
