...
- **Panic now outranks an in-flight apply, and the acceptance rollback refuses to run while panic mode is engaged.** Ending an open acceptance window used to be assumed safe to do from inside `Panic` — but cancelling a window does not stop its rollback, it *starts* it, and that rollback would otherwise land after the teardown and quietly re-arm the very table `panic` just tore down, in the worst case up to an hour later on the shipped default. The marker is now written and the audit entry recorded before the window is cancelled, and `rollback` checks the marker and refuses once it is set
...
[2.6.0]: https://github.com/jp1337/easywall/compare/v2.5.1...v2.6.0
