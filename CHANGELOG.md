...
- **A documented command that does not exist.** `configuration.md` told operators to run `easywall-core --write-config <path>` to get a commented default. Neither binary has ever had that flag — both take `-config` and `-version` and nothing else — so the command exits 2 with "flag provided but not defined". Replaced with what is true: the package installs both files already filled in, and the commented originals are `config/*.toml` in the repository. The page now also documents `--version`, which CI depends on and no page mentioned
