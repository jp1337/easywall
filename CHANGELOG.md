...
- **Two saves arriving together silently discarded one of them.** Every write is a read-modify-write of the one file holding all six rule lists, and the daemon handles each socket connection on its own goroutine — so both saves were built on the same read and the second wrote the first away. Measured at **187 of 200** trials. Two browser tabs is all it takes: add a blacklist entry in one, save a port in the other, and one of them is gone with "Changes saved" reported to both
