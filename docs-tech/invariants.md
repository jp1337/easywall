...
| `TestTheDocsSidebarRendersTheSearchContainer`, `TestTheSearchFieldIsHiddenWithoutJavaScript` | the search container is in the layout, and the built stylesheet hides it until a script says otherwise. Both halves fail silently: the page renders either way, and what is missing is a control, not a page. Both read the layout with its comments stripped: the first version searched for the bare string `data-js`, which the comment above the theme script also contains |
