...
| `TestTheAdvertisedLimitsAreTheOnesTheDaemonEnforces` | `options.html`'s `min`/`max` and both schemas' ranges are derived from `shared.FirewallLimits` | three sets of bounds, none in force: the page said 1–9999 everywhere, the schema said four different things, the daemon only checked `> 0` |
