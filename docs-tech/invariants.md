...
| `TestDaemonTests_StartIsOnlySpawnedByTheHelper` | `startDaemonGoroutine` is the only function in the package's test sources that spawns the real `Start` in a goroutine — seven tests need it, in any spelling. Renamed and widened in 2.15 from `…StartInAGoroutineIsAlwaysWaitedFor`, which forbade only `go func() { _ = d.Start() }()` | Two things are needed and each has gone missing once. **Waiting.** `Stop` closes the listener and waits on `d.wg`, which covers the boot restore, the reconciler and each served connection — deliberately **not** the accept loop. So `Stop` returns while the `Start` goroutine is still coming back from `Accept` and taking the quit branch. Measured at **3 runs in 200**. Four tests carried the discarding shape at once, and on 2026-08-30 one of them, `TestDaemonStart_RestoresAtStartup`, went red under `-race` on `main` — a goroutine touching `t.TempDir()`-scoped state while `testing` tore the frame down. **A slot in `d.wg`, held for exactly as long as `Start` runs.** Without it the accept loop calls `d.wg.Add(1)` at a counter of zero while the `Stop` on the test's own goroutine is inside `d.wg.Wait()`, and `sync.WaitGroup` reports that pairing on purpose — a read and a write of `wg.sema`, because `Add`'s documentation forbids the ordering. That is 2.15's flake, on `TestDaemonStart_RecordsAMarkerItCannotRead` and `…RestoresAtStartup`, and waiting for `Start` could never have covered it: the wait happens *after* `Stop`, and this races *inside* it. Reproduced at **7 process runs in 160** — sixteen copies of the package sharing eight cores under `-race`; it does not reproduce at all on an idle machine, nor pinned to one core, where the accept never happens. Releasing the slot one statement early, at the top of the cleanup, made it **worse** (15 in 192): `waitForSocket`'s dial is satisfied by the listen backlog, so the accept it provokes can land after that. The daemon is not the defect and was not changed either time: in production `Stop` is followed by process exit |
