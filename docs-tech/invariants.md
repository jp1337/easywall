...
| `TestBothPagesCountTheSameOutboundRequests` | the sentence "*N*, and this is the whole list." on `docs/_docs/security.md` and `docs/_docs/configuration.md` equals the number of files under `internal/` that build an outbound HTTP request | 2.20 added a third outbound request. Task 1 updated the configuration page, because `TestEveryConfigKeyIsDocumented` reads that page and no other; `security.md` went on saying **Two** above a two-row table while the feature shipped. A number written out in a sentence is invisible to a key-coverage test, and the two pages were bound by nothing but whoever remembered both. The third source is the code, so a *fourth* request cannot be added without extending both pages or recording why it is not on them |
