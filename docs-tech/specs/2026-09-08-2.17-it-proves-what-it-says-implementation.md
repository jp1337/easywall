...
- **No new Go dependency.** `mdlayher/netlink` is present via `google/nftables`; `golang.org/x/sys` is direct. Nothing else may be added.
  **Corrected during Task 4:** `unix.VETH_INFO_PEER` does **not** exist in
  `x/sys/unix` at any version — verified against v0.47.0. Declare it locally
  (`vethInfoPeer = 1`). `unix.IFLA_IFNAME`, `IFLA_LINKINFO`, `IFLA_INFO_KIND`,
  `IFLA_INFO_DATA` and `IFLA_NET_NS_PID` all do exist. And the peer nest is sent
  with the encoder's `Do`, not `Nested`: the kernel's
  `rtnl_nla_parse_ifinfomsg` skips a `struct ifinfomsg` before parsing, so the
  peer's attributes must sit behind sixteen zero bytes — `Nested` produces a
  message the kernel reads sixteen bytes into the middle of.
...
| `CheckRules` → `return nil` | **corrected during Task 2:** *not* all of them. Two of the tests assert **zero** findings, so a no-op checker passes them by construction. The implementer added two mutations to cover the gap — disabling the `precededByCtState` guard, and removing the recording seam, **without which the integration gate was green with `m.built` empty**. That second one is the important mutation in this task: the CI gate would have reported success while checking nothing |
...
`Close` kills the child and waits for it.

**Corrected during Task 4's review — this step first said "do not delete the links
explicitly", and that instruction was the defect.** The pair does die with the
peer's namespace, but *asynchronously*: `Close` returns long before the kernel's
`cleanup_net` runs, so the router-side link is still registered when the next
`NewHarness` sends its `RTM_NEWLINK`. Measured on an idle container: the name is
still taken for **~110 ms**, and under load netns teardown is batched and takes
much longer. A second harness in one process therefore failed 100 % of the time
with `netlink receive: file exists` — and not as `ErrNamespaceUnavailable`, so a
self-test run twice would report a hard harness failure rather than *unprovable*.
It would also have broken Task 5 deterministically, on the first `NewHarness` of
its second test.

So the asymmetry with `nftables_forward_test.go:63-68` is **not** justified: that
file deletes its router-side ends for exactly this reason, and so must this one.
Send one `RTM_DELLINK` for the router interface before `createVethPair`,
tolerating `ENODEV` (`errors.Is` works on the wrapped error). Deleting one end
takes its peer with it, so one message covers both. Verified: five back-to-back
harnesses with no wait. Renaming the links per-pid instead is **not** sufficient —
`10.77.9.1/24` is still held by the old router link, and `addAddress` also uses
`Create|Excl`.

The side effect belongs in a comment: it deletes any interface in this namespace
literally named `ewst-r`.
...
func NewNftablesManagerInNamespace(nsFD int) (*NftablesManager, error) {
	...
	m.adder = builtRecorder{m} // NOT `m`: *NftablesManager has no AddRule, so the
	// plan's original line did not compile. Corrected during Task 5.
	return m, nil
}
```
...
**Corrected during Task 5 — the uniqueness argument above was wrong, and a prover
built on it would have proven nothing.** The plan first said: open a connection
from the peer to a port the table accepts, have the router write to it, and the
reply can only get back through the established,related accept, "because the port
rule matched the SYN and this is not a SYN."

That is false. A connection *to* an open port carries that port as its
**destination on every packet of the connection**, not only on the SYN — so
`tcp dport N accept` matches the reply's data packets too, and the byte-reversal
mutation would have left the prover **green**. Two rules could produce the
outcome, which is exactly the confusion `reach_integration_test.go:184`'s comment
warns about and exactly what this task's own instruction — *name the one rule that
can produce this outcome* — exists to prevent. The plan violated its own rule.

The measured shape instead:

| Claim | Direction | Why only one rule can decide |
|---|---|---|
| 1 — a reply on an established connection passes | **peer → router** | the reply arrives on an **ephemeral** port with no port rule configured at all, so nothing but the established,related accept can admit it |
| 2, 3, 4 — open port accepts, closed port does not, blacklist wins | **router → peer** | nothing listens on the peer, so `ECONNREFUSED` versus a timeout is the signal, and a firewall verdict is distinguishable from an absent listener |

A consequence worth documenting rather than hiding: claim 1's control case needs an
inbound SYN accepted on the host side, so `easywall-core selftest` run by hand on a
host that already has a live table reports **`unprovable`**. That is honest, and it
is a second reason the shipped proof runs from a `oneshot` unit ordered
`Before=easywall-core.service` rather than against a running firewall.
...
| check the stamp before panic | **corrected during Task 6:** this does *not* redden that subtest. Its stamp is `passed`, and a passed stamp trips no stamp check wherever it sits in the order — so "panic beats the stamp" was asserted only in a comment. The case that reddens it is panic engaged **with** a failed stamp **and** a build finding, which Task 6 added |
...
- Modify: `.github/workflows/test.yml` — **added on 2026-09-08 after Task 5's review**

**Extra item for this task, found by Task 5's reviewer: nothing in CI fails if
layer C skips.**

No task in this plan owned a workflow file, and `test.yml:103` runs the
integration suite under `sudo` on `ubuntu-24.04`, so `CLONE_NEWNET` works and
layer C does run today. But **three `SKIP`s and three `PASS`es are the same green
tick.** The day a runner image, a sandbox change or a container step removes
CAP_SYS_ADMIN, the release's central proof silently stops running and CI stays
green — which is the same failure as Task 2's gate running green with an empty
accumulator, one level further out.

Add a step to `test.yml` after the integration run that fails on a skip. Either
grep the run's own output for a skipped selftest, or — better, because it cannot
drift from the test names — set an environment variable in CI that turns the
provers' `t.Skipf` into `t.Fatal`, so the suite itself refuses to be green
without the proof. Whichever route, verify it by mutation: make `NewHarness`
return `ErrNamespaceUnavailable` unconditionally and confirm the workflow step
goes red rather than green-with-skips.
...
| File | Change |
|---|---|
| ~~`docs-tech/protocol.md:9`~~ | **Moved to Task 7 on 2026-09-08.** `internal/shared/docs_coverage_test.go:282` — `TestEveryCommandIsDocumentedInBothPublishedAndTechnicalDocs` — fails the moment a command joins `AllCommandTypes` without a row in **both** `docs-tech/protocol.md` and `docs/_docs/architecture.md`. So the documentation for a new command is not deferrable to a docs task at all: the guard forces it in the same commit. This plan assigned it here and was wrong; Task 7 wrote both rows and the count |
...
> **Three times, so it is a rule and not an accident.** This plan gave a
> documentation task at the end of the branch files that guards force into the
> code commit: `docs_coverage_test.go:282` pulled `protocol.md` and
> `architecture.md` into Task 7, `TestEveryConfigKeyIsDocumented` and
> `TestEveryConfigKeyIsInTheSchema` pulled `configuration.md` and `docs/schemas/`
> into Task 9, and `TestAuditColourTableMatchesTheCode` pulled
> `features/audit-log.md` into Task 12. **This repository does not permit code and
> its documentation to live in separate commits.** A future plan should assign
> every guard-covered document to the task that changes the code it describes, and
> leave the documentation task only what no guard reaches: prose, screenshots and
> the changelog.

| ~~`docs/_docs/configuration.md`~~, ~~`docs/schemas/`~~ | **Moved to Task 9 on 2026-09-08, for the same structural reason `protocol.md` moved to Task 7.** `TestEveryConfigKeyIsDocumented` and `TestEveryConfigKeyIsInTheSchema` are derived from the config struct and go red the moment a field exists. Documentation for a new key is not deferrable to a docs task in this repository — **this plan made that mistake twice**, and both times a guard caught it in the same commit as the code |
