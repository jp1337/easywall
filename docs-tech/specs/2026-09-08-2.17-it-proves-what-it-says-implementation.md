...
`RuleCounters` keeps reporting reserved ids — health needs them — but `UsageStore.Collect` must never book one. Add the guard where liveness is decided, in `Collect` (`usage.go`), so it holds regardless of what `liveRuleIDs` returns:
