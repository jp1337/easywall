...
`Daemon.Start` runs the proof when the version **or** the kernel differs from the
record, and skips otherwise. The class of defect being prevented is a *build*
...
The proof runs before `RestoreCurrent` and cannot delay or prevent it.
...
Reserved ids begin with `_`, and `RuleCounters()` skips them. Without that,
`UsageStore.Collect` would try to book `_established` every tick, `liveRuleIDs()`
would not know it, and `Collect`'s prune loop would delete it again — harmless, but
a coupling nobody would expect to find. A guard pins that a reserved id never
reaches `usage.json`.
...
| `systemd/easywall-core.service` | becomes `Type=notify` with `WatchdogSec`. `sd_notify` is a write to `$NOTIFY_SOCKET` — about 40 lines, no dependency. It closes a real ordering weakness: under `Type=simple` the core is `active` before its socket exists, which is what `easywall-web.service`'s `After=` is waiting for |
