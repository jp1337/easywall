...
flowchart TD
    E{"Enforcing()?"}
    E -- no --> P{"panic mode engaged?"}
    E -- yes --> C
    P -- yes --> D1["fail — deliberately unfiltered"]
...
**What that condition costs, added on 2026-09-08 after Task 6's review.** It also
describes a port that *accepted* packets whose flow never reached ESTABLISHED:
an open port with no listener being scanned (the SYN is accepted, the kernel
sends RST, the conntrack entry closes, the next SYN is NEW again), a write-only
UDP receiver such as syslog or netflow, and the interval between an apply's
first inbound SYN and its ACK. On a host with no other inbound TCP at all, any
of those reports `degraded` and sends a reader hunting a defect that is not
there — the same harm the panic-before-counter ordering exists to avoid.

The class is narrow for a stateable reason: it needs inbound traffic and **no
completed inbound connection whatsoever**, and asking `/healthz` from a
monitoring system is itself a completed inbound connection, as is any admin
session. The tightening, if it ever proves necessary, is to require the
condition to persist across two reads — the defect is permanent where all three
false positives are transient. Deliberately not built: it needs state across
calls for a class this narrow.

**Corrected on 2026-09-09, during Task 12.** This section first said panic mode
reads `degraded`, and the flowchart showed panic as a branch reached only when
`Enforcing()` is true. Both were wrong, and Task 12's implementer found it rather
than implementing it: `Firewall.Panic` calls `nft.Reset()` (`restore.go:377`),
which leaves the input chain empty, and `Enforcing()` treats an empty input chain
as not enforcing — its own comment says so. So the machine returns in the first
branch and the `panic` reason was **unreachable on a real host**, while three
comments and this section promised otherwise.

The state is `fail`, not `degraded`. Under panic mode the machine is not filtering
at all, and `degraded` means *still filtering, something is off* — so `degraded`
would **understate**, and understating a firewall's state is the failure this
release exists to remove whichever direction it points. What was missing is not the
state but the **cause**: `not_enforcing` says the kernel carries no rules and
stops; `panic` says somebody unfiltered this machine deliberately and it stays that
way across a reboot until they run `easywall-core resume`. Those are different
things at three in the morning.

So panic selects the *reason* inside the not-enforcing branch. `/healthz` needs no
change — `fail` already answers 503, which is right: a container whose firewall is
deliberately down should read unhealthy, and a restart cannot re-arm it, because
`Daemon.Start` declines to filter while the marker is on disk.

**The divergence from `easywall-core status` survives and is stronger.** `status`
exits 0 under panic, because a machine somebody chose to unfilter is in a state somebody chose; that
exit code was ruled on and is not being reopened. `health` exits **2**.
