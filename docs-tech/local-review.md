...
| A demo server kept up across two `check:ui` runs used to fail the ports catalogue | Run 1's Pi-hole rows were still there for run 2, and asserting an absolute count reported *"picking Pi-hole added 4 TCP rows, expected 2"* — a real-looking regression worth a bisect. It no longer does: the check counts what *this* run added, not what is on the page in total. No workaround needed any more. |
| A stray `easywall-web` can still get checked instead of yours | `check:ui` now reads `bind_addr` out of the `web.toml` it already parses and prints the URL and the config it chose, so a moved `EASYWALL_DEMO_ADDR` is followed (`EASYWALL_URL` still overrides both). What that does not catch is a second, unrelated `easywall-web` already listening on the address your config also names. `pgrep -x easywall-web` first — note `pkill -f easywall-web` kills your own shell, because `-f` matches pkill's own command line. |
...
The packet-log tests bind an NFLOG group, which needs `nfnetlink_log` loaded on
the host — a container shares the host's kernel. `sudo modprobe nfnetlink_log`
before the run guarantees it; without the module those two tests skip with that
reason. CI runs under `sudo` with
