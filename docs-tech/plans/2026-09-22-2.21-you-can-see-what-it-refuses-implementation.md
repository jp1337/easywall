...
func TestDecode_MetadataFromTheAttribute(t *testing.T) {
	...
	idx, hook, ct := uint32(3), uint32(1), uint32(2)
...
// entry without its ct state is worth more than no entry.
func (p *PacketLog) listen(ctx context.Context, group uint16) error {
	var err error
	for _, flags := range []uint16{nflog.FlagConntrack, 0} {
		var nf *nflog.Nflog
...
type blockedRows struct {
	...
	// view, and every row action comes back to it.
	//
	// template.URL, not string: in a URL attribute after "?", html/template
	// query-escapes a plain string, so "port=22&proto=tcp" would arrive as
	// "port%3d22%26proto%3dtcp" — one parameter the handler cannot read. A
	// template.URL is normalised instead of escaped.
	Query template.URL
}
...
// queryURL is filterQuery marked safe for a URL attribute. Safe because it is
// url.Values.Encode over fields Validate has already accepted.
func queryURL(f shared.PacketLogFilter) template.URL {
	return template.URL(filterQuery(f)) // #nosec G203 -- url.Values.Encode of a validated filter
}
...
func (s *Server) handleBlocked(w http.ResponseWriter, r *http.Request) {
	...
	data := &blockedData{
		...
		Rows:     blockedRows{Query: queryURL(f)},
...
func (s *Server) handleBlockedRows(w http.ResponseWriter, r *http.Request) {
	...
	rows := blockedRows{Entries: []shared.PacketLogEntry{}, Query: queryURL(f)}
