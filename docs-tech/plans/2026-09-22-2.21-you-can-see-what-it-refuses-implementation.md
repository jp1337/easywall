...
- Modify: `internal/core/config.go` (accessors, `Validate`, `Reload` warning, `PacketLogPath`, `saveLocked`)
...
Expected: FAIL in several places — `configuration.md does not document "nflog_group"` (and `entries`, `persist`), the schema, `config/easywall.toml does not mention`, and `docs/_docs/architecture.md does not document command GET_PACKET_LOG`. Steps 7–8 close them.
...
`docs/_docs/architecture.md:65`: "Twenty-two message kinds" → "Twenty-three message kinds".
...
- [ ] **Step 10b: `saveLocked` must restore `[packet_log]` from the file too — failing test first**

Without this, `saveLocked` (`internal/core/config.go:497`) encodes `snapshot := c.CoreConfig`, which carries the *running* `PacketLog` — bound at start, exactly like the three paths above it. An operator who edits `[packet_log]` directly for the next restart, and then triggers a `Reload` before that restart (a `SIGHUP`, or any other section's own save calling it), has that edit silently erased the moment anyone next presses Save on Options, Network or System: `saveLocked` writes the stale running value back over it. It is the same bug `TestEnvOverlayNeverReachesTheConfigFile` exists to catch on the three paths, on a fourth field nothing currently protects.

Add to `internal/core/config_packetlog_test.go`:
...
(`writeTempCoreConfig`, `validCoreConfig` and `os` are already in `internal/core/config_test.go`'s package `core`, so this needs no new import beyond what `config_packetlog_test.go` already has.)

Run: `go test ./internal/core/ -run TestSavePreservesAPacketLogEdit -v`
Expected: FAIL — `nflog_group on disk = 12227 after a Save, want the operator's edit (5000) kept`.

In `internal/core/config.go`'s `saveLocked`, add a fourth restore line after `snapshot.LogDir = c.fileConfig.LogDir`:
...
Mutation: delete the `snapshot.PacketLog = c.fileConfig.PacketLog` line. `TestSavePreservesAPacketLogEditMadeDirectlyInTheFile` must go red. Restore.
...
Expected diff: one new line in the direct `require` block, `github.com/florianl/go-nflog/v2 v2.3.0`, and nothing new under `// indirect` — its dependencies (`mdlayher/netlink`, `golang.org/x/sys`, `github.com/google/go-cmp`) are already in this module's graph. If anything else appears, stop and report it: the spec's cost table says *no new transitive tree*.
...
Add `"encoding/json"`, `"os"`, `"path/filepath"` and `"strings"` to the import block at the top of the file — this step is the first to call `json.Marshal`, `os.WriteFile`/`os.ReadFile`/`os.Stat`, `filepath.Join` and `strings.Count`.
...
Add `"bufio"`, `"encoding/json"`, `"errors"`, `"fmt"`, `"log/slog"`, `"os"`, `"path/filepath"` and `"sync"` to the file's import block — Step 3's decoder needed none of them, and this half is what uses every one.
...
- Modify: `internal/core/packetlog_test.go` (`TestHookCountsWhatItCannotDecode`)
...
- Produces: `type logSink struct{ nflog bool; group uint16 }`; `func (m *NftablesManager) SetLogSink(s logSink)`; `func (p *PacketLog) listen(group uint16) (stop func(), err error)`; `func (d *Daemon) startPacketLog()`; `Daemon.listenPacketLog func(p *PacketLog, group uint16) (func(), error)`; `Daemon.packetsStop func()`; `GET_PACKET_LOG` answered from the ring.
...
- [ ] **Step 3: The listener** — append to `internal/core/packetlog.go`, and add `context`, `errors` and `net` to its imports

`listen` takes no `ctx`: it owns one internally and hands back a `stop` func that cancels it and closes the socket **synchronously**, so a caller that has just called `stop` can immediately rebind the same group without racing a goroutine that has not finished closing yet — see Task 3's daemon wiring and integration tests below, which both depend on that.
...
- [ ] **Step 3b: `hook` counts what it cannot decode — failing test first** (spec §8; Review Focus 4's sibling for the live path, not the replay)

Add to `internal/core/packetlog_test.go` (add this file to Task 3's Files list as modified):
...
Run: `go test ./internal/core/ -run TestHookCounts -v`
Expected: FAIL, then PASS once Step 3's `hook` above exists — `hook` already calls `p.countDiscarded()`; this step only adds the test that proves it.

Mutation: in `hook`, delete the `p.countDiscarded()` call. `TestHookCountsWhatItCannotDecode` must go red. Restore.
...
2. In `logExprs`, add `log.Key |= 1<<unix.NFTA_LOG_FLAGS; log.Flags = expr.LogFlagsNFLog` inside the `if sink.nflog` — setting `Flags` alone is never marshalled onto the wire (`expr.Log.marshalData` gates every attribute on its own `Key` bit), so the mutation must set the bit too or it is invisible everywhere. `TestLogExprs_TheNFLOGSinkNamesTheGroup` must go red, and in podman `TestIntegration_ADroppedPacketArrivesTyped` must fail at `Apply` with `EINVAL`. Restore. **Record the integration outcome in the task report** — it is the proof behind the plan's correction to the spec.
3. At call site `:2737`, pass `logSink{}` instead of `m.logSink`. `TestEveryLogRuleUsesTheManagersSink` must go red. Restore.
4. Change `logPrefixPortScan` to `"easywall port-scan: "`. `TestLogPrefixesMapToTheRulesTheyName` must go red — a real port-scan packet would otherwise render as "other" and never match `rule=portscan`. Restore.
...
- Modify: `internal/web/server.go` (routes, after the `/log` pair; `templateFuncs` gains `fullTime`)
- Modify: `internal/web/server_viewfuncs_test.go` (`fullTime`'s test, beside `shortTime`'s)
...
- Modify: `docs/_config.yml` (nav entry — moved here from Task 7 so `TestEveryDocsPageIsInTheNav` stays green the moment `blocked-traffic.md` exists)
...
**Interfaces:**
- Consumes: `CoreClient.GetPacketLog`, `CoreClient.GetOptions`, `shared.PacketLog*`, `FirewallOptions.LogsAnything` (Task 1).
- Produces: `type blockedData struct`, `type blockedRows struct{ Entries []shared.PacketLogEntry; Query string; Logging, Listening, Filtered bool; Since time.Time }`, `func blockedFilter(q url.Values) (shared.PacketLogFilter, bool)`, `func filterQuery(f shared.PacketLogFilter) string`, `func fullTime(t time.Time) string`, templates `blocked.html` and `blocked_rows`. Task 5 adds a form per row inside `blocked_rows` and relies on `.Query` being there.
...
The `{{range}}`'s `{{else}}` runs with `.` still the outer `blockedRows`, so `.Logging`, `.Filtered`, `.Listening` and `.Since` resolve there without a `{{with .Result}}` wrapper — this is `blockedRows`, not `PacketLogResult`. `blocked_empty_quiet` is guarded by `.Listening` (and comes last, after the other two): when the core is not listening at all, nothing is being received, so saying "nothing has been refused" would be false — the amber `blocked_not_listening` alert above the table already says why, and the row itself renders empty rather than repeat it.

`fullTime`, beside `shortTime` in `internal/web/server.go`'s `templateFuncs`:
...
Register it in `templateFuncs`'s map, beside `"shortTime": shortTime,`:

```go
		"fullTime":   fullTime,
```

Add its test to `internal/web/server_viewfuncs_test.go`, beside `TestShortTime`:
...
```json
  ...
  {"id": "blocked_detail_mark", "translation": "Mark"},
...
```json
  ...
  {"id": "blocked_detail_mark", "translation": "Markierung"},
...
`docs/_config.yml`, in the features nav directly before *Audit Log* (moved here from Task 7: `TestEveryDocsPageIsInTheNav` requires every page under `docs/_docs` to have a nav `path:`, and that test runs in this task's Step 7 — a nav entry that only arrives in Task 7 would fail it here first):
...
git add internal/web internal/shared/docs_coverage_test.go web/templates web/src/app.css \
  web/static/style.css locales/en.json locales/de.json docs/_docs/features/blocked-traffic.md docs/_config.yml
...
`flashOf` reads the flash out of the session cookie the redirect set — `SessionName` is the store's cookie name (declared `auth.go:21`, used the same way at `sessionrevoke.go:107`), the pattern `handler_passkey_test.go:815` already uses.
...
| 5 | In `lockoutRefusal`, return `""` from the `oErr != nil \|\| nErr != nil` branch instead of `"blocked_refused_unknown"` (returning `""` from a check on `opts` alone either leaves the test green or panics on `*nets`, since settings is what actually fails here) | `TestStageRefusesWhenTheVerdictCannotBeAsked` |  |
...
Demo server, Chrome extension, Full HD, both themes, both languages: stage a whitelist, a blacklist and a port from three rows; watch the header counter go 1 → 2 → 3 and the Apply link land on the apply screen listing all three.

The lockout refusal itself is **not** renderable here: the demo's rows are all on documentation addresses, and a local session's peer is loopback, which `shared.Reachable` always answers *open* regardless of the blacklist — there is no address the demo can be made to refuse blacklisting. `TestStageRefusesToBlacklistTheOperator` and its siblings are the only proof this release has; the amber-not-red rendering of `blocked_refused_lockout` and `blocked_refused_proxy` is checked in Task 8 Step 4 against a build pointed at a real, non-loopback peer instead.
...
- Modify: `docs/_docs/security.md`, `docs/_docs/roadmap.md`, `CHANGELOG.md` → `docs/_docs/changelog.md` (`docs/_config.yml`'s nav entry moved to Task 4, so `TestEveryDocsPageIsInTheNav` does not fail between the two tasks)
...
3. **Read a row** — the columns; the drill-down (TCP flags, connection state, TTL, exact time, mark); one screenshot, using the include `apply.md` uses:
   ```
   {% include themed-figure.html base="/assets/img/screens/blocked" ext="png"
      alt="…" %}
   ```
   (`docs/_includes/figure.html` does not exist — `themed-figure.html` is the one every feature page has), `alt` under 30 words.
...
The nav entry (`docs/_config.yml`) was already added in Task 4 Step 6, so `TestEveryDocsPageIsInTheNav` would not fail here — nothing more to do for it.
...
`docs-tech/dependencies.md`: one line under the direct dependencies — `florianl/go-nflog/v2` — MIT, v2.3.0, reads NFLOG in `easywall-core`; its dependencies (`mdlayher/netlink`, `golang.org/x/sys`, `github.com/google/go-cmp`) are already in this module's graph.
...
- Re-take: `docs/assets/img/screens/dashboard-*.png` (the sidebar gained an item — every screenshot with a sidebar shows it, so **every** file in `DEFAULT_SCREENSHOT_PAGES` is re-taken, and so are `two-factor-setup-*.png` and `two-factor-codes-*.png`, taken on `/password` by their own flow rather than by visiting a path in that list)
...
With no page names, this is `takeFullScreenshotSet` (`scripts/ui-check.mjs`), not only `DEFAULT_SCREENSHOT_PAGES`: it also runs `takeLoginScreenshot`, `takeEnrolmentScreenshots`, `takeWizardScreenshots` and `takeVerifyScreenshot`. The new nav item shows on every page that carries the sidebar, and that includes `two-factor-setup-*.png` and `two-factor-codes-*.png` — both taken signed in, on `/password`, inside `takeEnrolmentScreenshots` — not only the files in `DEFAULT_SCREENSHOT_PAGES`.

Expected: `blocked-light.png` and `blocked-dark.png` are new; every file in the default set, plus `two-factor-setup-*` and `two-factor-codes-*`, is modified (the sidebar). `login-*`, `firstrun*` and `two-factor-verify-*` have no sidebar and are not expected to change — confirm that with `git status` too, rather than assuming it. Open `blocked-light.png` and `blocked-dark.png` and look at them: rows present, one drill-down open is **not** required, no clipped address.
...
| each row action stages (whitelist, blacklist, open port) | ☐ | ☐ |
...
Neither is the lockout refusal itself: every demo peer is loopback, which `shared.Reachable` always answers *open*, so there is no address the demo can be made to refuse blacklisting (Task 5 Step 6 explains why). `TestStageRefusesToBlacklistTheOperator` and its siblings are the proof of the refusal logic; that `blocked_refused_lockout` and `blocked_refused_proxy` render amber, and that the German text fits its alert without overflow, is instead checked by pointing a local build at a non-loopback peer, or not at all — say which in the report.
