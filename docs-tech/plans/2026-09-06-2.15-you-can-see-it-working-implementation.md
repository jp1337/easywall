...
| **A4** | The baseline reset happens **after `nft.Apply` returns**, on the success branch only | §4 says "after collecting". `nft.Apply` refuses before touching the table when `ValidateRules` fails, and a baseline zeroed at that point re-books the entire lifetime of every live kernel rule at the next collect. The guard §8 asks for is unchanged: remove the reset, it goes red. **Corrected in the final review:** this row originally reasoned "after the call, both outcomes are a rebuilt table", which is not true. `nft.Apply`'s error is two situations — a refusal before the table is touched, and a failure after the ruleset is committed — and they want opposite things, with nothing in the error to tell them apart. So the reset stays on the success branch, and what covers the post-commit failure is that the error branch calls `rollback`, whose own kernel write resets the baselines, with `Collect`'s below-baseline branch behind it |
