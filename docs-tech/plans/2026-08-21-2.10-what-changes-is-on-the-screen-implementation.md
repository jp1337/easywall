...
were largely executed against it. All three shipped in the real code and are
recorded here — openly, rather than left for a reader to notice the plan
disagrees with `internal/shared/reach.go` — because the sections below that
predate them (Task 3's original chain table and code walkthrough, Task 7's
first locale draft) still read the way they were written at the time and are
superseded by these three points, not by a silent second edit of every line
that quoted the old wording:
...
2. **`ReasonDockerNetwork` split in two.** It now covers only the *configured*
   ...
   therefore carries thirteen members, not twelve.
3. **`Reachable` takes a seventh argument, `local`**, decided by the caller from
   ...
   *arrival interface* (`iifname "lo"`), not the address, so a connection to
   ...
   working connection was about to be cut. Task 3's `Interfaces` list already
   named this argument; its Step 1 test table and Step 3 code walkthrough below
   were written before it and call `Reachable` with six arguments — the shipped
   signature and `TestReachable`'s real table both take seven.
