...
- **`unknown` beats a guess.** Every case `Reachable` cannot decide returns
  `unknown` with a reason. A false *blocks new connections* costs trust in the
  one warning that must be believed; a false *reachable* costs a lockout.
...
## Amendments made after review, before release

Three maintainer rulings arrived after this plan was written and Tasks 1–8
were largely executed against it. All three shipped in the real code
(`internal/shared/reach.go`, `internal/core/reach_integration_test.go`,
`locales/en.json`/`de.json`). Task 3's and Task 4's code listings below were
written before these rulings and, on a first pass, corrected only where the
signature itself appears — the call sites, the enum, and one test's explicit
reason list still disagreed with the code they were supposed to record. They
have since been corrected in place, listing by listing, rather than left to
silently drift from `internal/shared/reach.go`:

1. **The blocked verdict reads `blocks new connections` / `blockiert neue
   Verbindungen`, never "cuts your connection" / "trennt deine Verbindung".**
   The old wording made exactly the claim §3's *What the wording must not
   claim* forbids: applying rules does not touch conntrack, so the operator's
   own connection stays `ESTABLISHED` and keeps answering. Read "cuts your
   connection", apply, see the page still working, conclude the warning was
   wrong, confirm — and a lockout is confirmed on the strength of a sentence
   that overclaimed. Task 7's locale draft below is corrected to match.
2. **`ReasonDockerNetwork` split in two.** It now covers only the *configured*
   `docker.custom_networks` case, which is `open`; the auto-detected bridge-pool
   case is a new `ReasonDockerBridge`, which is `unknown`. One sentence could
   not be true of both — it told the operator the networks "cannot be listed
   from here" beside a green verdict that had just listed one. `ReachReason`
   therefore carries thirteen members, not twelve: Task 3's `ReachReason`
   constant block, `AllReachReasons`, `TestAllReachReasonsIsComplete`'s explicit
   list, `TestReachable`'s auto-detected-bridge case, and the Docker branch in
   the `Reachable` walkthrough are all corrected below to declare and return
   `ReasonDockerBridge` for that case; Task 7's locale draft gains the matching
   `reach_docker_bridge` key in both languages.
3. **`Reachable` takes a seventh argument, `local`**, decided by the caller from
   `net.InterfaceAddrs()`. The kernel's first input rule accepts on the
   *arrival interface* (`iifname "lo"`), not the address, so a connection to
   any address the host holds is accepted before a rule is consulted — without
   this, an operator at the console on the host's own LAN address was told a
   working connection was about to be cut. Task 3's `Interfaces` list and its
   Step 3 function signature already named this argument — that part was never
   stale. What was: Step 1's `TestReachable` case struct had no `local` field
   and its call site passed six arguments; Task 4's veth test call did the
   same. Both are corrected below to seven, matching `TestReachable` and
   `TestIntegration_ReachableAgreesWithTheKernel` as shipped.
...
```bash
npm run build:css
grep -o '\.flow{[^}]*}' web/static/style.css               # expect the rule, twice
grep -c 'td>\.flow' web/static/style.css                   # expect 1
grep -c 'cell-flow' web/static/style.css                   # expect 0
```

Expected: `.flow{…display:flex}` and `.flow{flex:1;min-width:0}`, then `1`, then
`0`. Grep for the selector and print the whole rule rather than for a
declaration in a fixed position: lightningcss reorders declarations inside a
rule while minifying, so `grep 'flow{display:flex'` answers `0` on a stylesheet
that has the rule. A missing rule means Tailwind dropped it and the fix did not
ship.
...
| `custom` | position among the **effective** lines | `"#3"` | the rule line, on added and removed | the rule line, before → after |
...
- Produces:
  ...
  - `type ReachReason string` — the thirteen constants below (`docker_network`
    and `docker_bridge` count as two — see *Amendments* above), and
    `AllReachReasons`
  - `func Reachable(r Rules, o FirewallOptions, n NetworkSettings, src netip.Addr, port uint16, proxied, local bool) (ReachVerdict, ReachReason)`
...
| 6a | Docker bridge accepts — named in `docker.custom_networks` | `open` · `docker_network` |
| 6b | Docker bridge accepts — auto-detected pool | `unknown` · `docker_bridge` |
...
- **Auto-detected Docker bridges get their own reason, `docker_bridge`,
  distinct from a named network's `docker_network`.** One sentence used to
  cover both, and it could not honestly be true of either: a network the
  operator named in the settings is one this process can and does list; the
  auto-detected pool is settled in the core at apply time and cannot be. Bounded
  to `172.16.0.0/12`, which is where Docker's default address pool allocates
  from — outside that range an auto-detected bridge cannot be the explanation,
  so an ordinary `192.168.x` or `10.x` LAN address still gets a real answer
  instead of a shrug.
...
func TestReachable(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "an auto-detected bridge is unknowable from here",
			...
			// A different reason from the named-network case above: this one
			// cannot be listed from here, so it is unknown rather than open.
...
func Reachable(r Rules, o FirewallOptions, n NetworkSettings,
	...
	// is routed over lo and accepted before a rule is consulted.
	// src.IsLoopback() catches only the 127.0.0.0/8 and ::1 spelling of that, so
	// the caller — which can enumerate this host's interfaces, and this package
	// deliberately cannot — supplies the rest. Without it, an operator at the
	// console reaching the interface on the host's own LAN address is told their
	// working connection is about to be cut.
	//
...
and the helper the loopback step depends on:
...
Imports to add: `net`, `net/netip`, `strconv`.
...
```json
  ...
  {"id": "apply_verdict_blocked", "translation": "blocks new connections"},
  ...
  {"id": "reach_docker_network", "translation": "Your address is in a Docker network named in the settings, which is accepted."},
...
```json
  ...
  {"id": "apply_verdict_blocked", "translation": "blockiert neue Verbindungen"},
  ...
  {"id": "reach_docker_network", "translation": "Deine Adresse liegt in einem in den Einstellungen genannten Docker-Netz und wird akzeptiert."},
...
```html
              <form method="POST" action="/apply/start">
                ...
                {{end}}{{end}}
                {{/* A verdict of "blocks new connections" replaces the primary
...
- **What changes is on the screen, before the apply.** `/apply` lists every
  ...
  "blocks new connections" would cost the trust the true one needs. The verdict is
