...
| 14 | — | `debian/postinst`, `debian/control`, `systemd/easywall-web.service`, `Dockerfile`, `docker/entrypoint.sh`, `internal/core` (`last_apply`, start check), `internal/web` (data paths), `.github/workflows/build.yml` (build-deb, build-docker), `docs-tech/{threat-model,packaging,invariants}.md` |
...
| 8 | new drop-reason code in `internal/shared/packetlog.go` (after `Remedies`), calls `InAnyEntry` | `InAnyEntry` body only, signature unchanged | No textual overlap. `DropReason` now reads a mapped network the way the kernel does |
...
A defect proven at v2.21.1, not caused by this plan, and fixed here (`CLAUDE.md`, *A release fixes what it caused*; the repository's rule to fix what a pass finds). Runs in **wave 2, after Task 1 and before Task 3**. Task 3's text changes as a result: see *Task 3, amended* at the end of this section.
...
| A non-root file among the core's names | given to root (`chown -h`), named on stderr | Its owner could otherwise go on writing it in place whatever the directory's mode. `rules.json` cannot be told apart from yours, and deleting it would enforce an empty set at the next boot |
...
- Modify: `internal/shared/systemd_units_test.go` (one test, one import), `systemd/easywall-web.service:34`
- Modify: `debian/postinst:29-39`, `Dockerfile:66-78`, `docker/entrypoint.sh` (a block before line 63)
...
- Consumed by Task 3 (its entrypoint `test -w` loop uses `$DATA/web`; its assertion D changes — replacement text below) and Task 9 (one changelog entry, under *Security*).
...
Expected: build failure `undefined: dataDirIsShared`. With `datadir.go` from Step 2 present but `firewall.go` unchanged: `TestTheLastApplyMarkerDoesNotWriteThroughALink` FAILs (`the link's target now reads "2026-…"`) and `TestTheLastApplyMarkerIsNotReadThroughALink` FAILs; `TestEveryCoreDataPathIsDirectlyInDataDir` passes at HEAD — it is a guard, verified by mutation M4.
...
func readLastApply(path string) time.Time {
	f, err := os.OpenFile(path, os.O_RDONLY|syscall.O_NOFOLLOW, 0) // #nosec G304 -- path is built from the daemon's own config
	if err != nil {
		return time.Time{}
	}
...
```sh
    ...
    #
    # The install -d above made it 0750, so from here on only root changes what
    # is in it; prerm stopped both services, so nothing races this. What an
    # older layout left behind is dealt with now, and each pass is a no-op on
    # the next upgrade:
    ...
    #   - anything else not owned by root is given to root. Its owner could
    #     otherwise go on writing it in place, whatever the directory's mode.
    ...
    for p in "$data"/* "$data"/.[!.]*; do
        ...
        totp_replay.json|passkeys.json|passkeys.json.corrupt|version_cache.json|telemetry.json)
          if [ -f "$p" ]; then
            if [ -e "$data/web/$f" ] || [ -L "$data/web/$f" ]; then
              rm -f "$p"
            elif ! { mv "$p" "$data/web/$f" && chown -h easywall:easywall "$data/web/$f"; }; then
      ...
      if [ "$(stat -c %u "$p")" = 0 ]; then
        ...
        echo "easywall: $p was not owned by root; it is now. Review it" >&2
        chown -h root:root "$p" || echo "easywall: could not give $p to root" >&2
      fi
    done
```
...
#   - anything else not owned by root is given to root, whose owner could
#     otherwise go on writing it in place whatever the directory's mode.
...
for p in "$DATA"/* "$DATA"/.[!.]*; do
    ...
    case "$f" in
        totp_replay.json|passkeys.json|passkeys.json.corrupt|version_cache.json|telemetry.json)
            if [ -f "$p" ]; then
                if [ -e "$DATA/web/$f" ] || [ -L "$DATA/web/$f" ]; then
                    rm -f "$p"
                elif mv "$p" "$DATA/web/$f"; then
                    chown -h easywall:easywall "$DATA/web/$f" || warn "could not set the owner of $DATA/web/$f"
                else
    ...
    if [ "$(stat -c %u "$p")" = 0 ]; then
        ...
        warn "$p was not owned by root; it is now. Review it"
        chown -h root:root "$p" || warn "could not set the owner of $p"
    fi
done
```
...
```yaml
      ...
      - name: The entrypoint takes a 2.21 data directory apart safely
        run: |
          ...
          # writes, web/ itself a link to /etc, and a panic marker.
...
Run: `~/go/bin/actionlint .github/workflows/build.yml` — the same findings as `main` (the `ubuntu-26.04` label and the two SC2034 `for i in` loops in `build-deb`), nothing new.
...
| M2 | `os.O_RDONLY\|syscall.O_NOFOLLOW` → `os.O_RDONLY\|0*syscall.O_NOFOLLOW` | `TestTheLastApplyMarkerIsNotReadThroughALink` |
...
| S3 / E3 | `chown -h root:root "$p"` → `:` | the replaced file stays writable by the web user |
| S4 / E4 | the link branch's `rm -f "$p"` deleted | the planted `last_apply` link survives |
| S5 / E5 | the `mv` into `web/` → `false` | the TOTP replay step is lost |
...
- :114-116 →
  ...
  # /var/lib/easywall/web by default), then restart easywall-web.
  ```
...
| Wall | Where | Covers |
|---|---|---|
| the mode | `debian/postinst`, `Dockerfile`, `docker/entrypoint.sh` | every layout the package or the image sets up. Both scripts also take an old layout apart: a link is removed unfollowed, the web's files move to `web/`, a `panic` not owned by root is removed, anything else not owned by root is given to root |
...
- What an attack before the upgrade already did to `data_dir`. postinst and the
  entrypoint give a replaced `rules.json` to root and name it on stderr; they
  cannot tell it from yours. A manual install keeps its mode until the operator
  runs the two commands in `installation/manual.md`, and the core says so at
  every start until then.
- A hard link to a root file planted under a core name, which the migration's
  `chown` would then re-own. Only possible with `fs.protected_hardlinks=0`;
  systemd's `50-default.conf` sets it to 1.
```
...
- A new section before `## Neither config is shipped under its own name`:
  ...
  | anything else not owned by root | gives it to root, and names it |
...
git add internal/core/firewall.go internal/core/datadir.go internal/core/datadir_test.go internal/core/daemon.go \
  ...
  internal/shared/systemd_units_test.go systemd/easywall-web.service debian/postinst Dockerfile docker/entrypoint.sh \
...
Check: `toolbox run --container fedora-toolbox-42 dpkg-deb -f <built .deb> Depends` lists `adduser`. Commit with this task's last commit, subject `fix(debian): depend on adduser, which postinst calls`.
