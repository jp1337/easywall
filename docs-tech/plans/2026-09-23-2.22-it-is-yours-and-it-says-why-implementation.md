...
- **Locale keys are inserted after the key group each task names** (`firstrun_*` T1, `opt_*` T6, `blocked_rule_*` T8), so parallel tasks do not touch the same lines. Never reserialise a locale file: targeted edits only, `wc -l` and key order unchanged except for the added keys.
...
|---|---|---|
| 1 | 12271 | `internal/web/handler_firstrun*.go`, `server.go`, `web/templates/firstrun.html`, `locales` (`firstrun_*`), `scripts/ui-check.mjs` (wizard), `debian/postinst`, `docs/_docs/installation/{first-run,demo}.md`, `docs-tech/threat-model.md` |
...
In `handleFirstRunPOST`, between the `answers := &firstRunData{…}` literal and `password := r.FormValue("password")` (line 137):
...
```go
	...
	if !s.setupTokenMatches(r.PostFormValue("setup_token")) {
		...
		slog.Warn("first run: refused a submission without the right setup token", "client", addr)
...
// process printed at start. Both sides go through decodeTOTPSecret, so a token
// pasted with its spaces, in lower case or with dashes still matches, and the
// comparison is on the decoded bytes, in constant time. No token held — the
// process started with an account — matches nothing.
...
In `NewServer`, after `s.passkeyCount = func() int { return len(s.passkeys.all()) }` (line 315):
...
| M3 | `return s.setupToken == given` instead of the compare | `TestFirstRunAcceptsTheSetupTokenHoweverItIsPasted` (3 of 4 spellings), `TestTheSetupTokenIsComparedInConstantTime` |
...
```json
  ...
  {"id": "firstrun_token_help",    "translation": "Proves you can read this server's log, where easywall printed it at start. Docker: `docker compose logs easywall | grep 'setup token'` · Debian: `journalctl -u easywall-web -g 'setup token'`. A restart prints a new one."},
...
```json
  ...
  {"id": "firstrun_token_help",    "translation": "Beweist, dass du das Log dieses Servers lesen kannst — dort hat easywall es beim Start ausgegeben. Docker: `docker compose logs easywall | grep 'setup token'` · Debian: `journalctl -u easywall-web -g 'setup token'`. Ein Neustart gibt ein neues aus."},
...
```sh
      ...
      if grep -Eq '^[[:space:]]*password[[:space:]]*=[[:space:]]*""' /etc/easywall/web.toml; then
        echo "easywall: open https://<this host>:12227/firstrun — the setup token it asks for: sudo journalctl -u easywall-web -g 'setup token'"
      fi
```
...
```yaml
          ...
          docker exec easywall-test journalctl -u easywall-web --no-pager | grep -q 'setup token' \
            || { echo "::error::no 'setup token' line in the easywall-web journal — nobody can complete /firstrun"; exit 1; }
```

(`grep`, not `journalctl -g`, so the CI assertion does not depend on journalctl's PCRE2 build.) The `GET /firstrun → 200` check above it still holds: the page renders for anyone; only the POST is gated.
...
|---|---|
| Docker | `docker compose logs easywall \| grep 'setup token'` |
| Debian | `sudo journalctl -u easywall-web -g 'setup token'` |
...
`docs/_docs/installation/docker.md:26`: `Three things to expect on that first page:` → `Four things to expect on that first page:`, and a first row in that table:
```markdown
...
function before `TestDockerConfig_AnAbsentKeyLeavesPublishedPortsOpen`.)
...
`internal/web/daemon_dispatch_test.go:169`,
...
`EASYWALL_CI_STATE` is set per step because `runner.temp` is not available in `jobs.<id>.env`. The state directories live under `runner.temp`, outside the checkout, so assertion K's `git status` sees only what compose itself created in the tree.

Run: `~/go/bin/actionlint .github/workflows/build.yml` — only the findings `main` already has (the `ubuntu-26.04` label, SC2034 at the two `for i in` loops in `build-deb`).
...
> and puts it into the shape the Debian package installs — `web.toml` and `ssl/`
> to the container's `easywall` user (uid 100, gid 101), `easywall.toml` to root.
...
```bash
git add docker-compose.yml docker-compose.ci.yml .gitignore Dockerfile docker/entrypoint.sh \
  .github/workflows/build.yml docs/_docs/installation/docker.md docs/_docs/environment.md \
  docs-tech/ci-and-release.md
...
func TestHealthzAdmitsItsOwnAddressAndNoOther(t *testing.T) {
	for _, tc := range []struct {
		...
		{"own address", "10.0.0.9:40000", []string{"127.0.0.1/8"}, http.StatusOK},
		{"a neighbour", "10.0.0.5:40000", []string{"127.0.0.1/8"}, http.StatusNotFound},
		{"own address, endpoint off", "10.0.0.9:40000", []string{}, http.StatusNotFound},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			req = req.WithContext(context.WithValue(req.Context(), http.LocalAddrContextKey,
				&net.TCPAddr{IP: net.ParseIP("10.0.0.9"), Port: 12227}))
...
func healthTarget(bindAddr string) (url, host string, err error) {
	...
	return "https://" + net.JoinHostPort(host, port) + "/healthz", host, nil
}
...
Run: `go test ./internal/web/ -run 'Health'` — PASS, all 17 (14 existing).
...
# is its own local address — so moving bind_addr needs no change here. An
# empty list switches that off too.
```
...
ownership callout, the `docker run -v` examples) is Task 3's diff. The steps
below anchor on text those lines do not touch, so the two diffs cannot
collide; if Task 3 lands first, every anchor quoted below still matches
byte for byte.
...
In the "Three things to expect on that first page" table (`docker.md:29–33`),
add a fourth row:
...
| This host's containers | already trusted — `docker.enabled = true` ships by default; see [Docker Coexistence]({{ '/docs/features/docker/' | relative_url }}) to turn it off or change what it accepts |
```
...
easywall-web -g 'setup token'`, or `docker compose logs easywall | grep
'setup token'`. The rules, the audit log and every other setting are
...
Insert a new section into `docker.md`, after the `./config` ownership
callout and before "## What must persist" (i.e. immediately after the
paragraph ending "**Editing those files on the host afterwards needs
`sudo`.**", and before the `## What must persist` heading — this is text
Task 3 does not touch, so the anchor holds regardless of merge order):
...
caveat (`reverse-proxy.md:111–116`):
...
(Only the sentence spanning the old lines 112–114 changes; the rest of the
...
func TestDemoICMPRowsCarryTheirType(t *testing.T) {
	...
	for _, proto := range []string{"icmp", "icmpv6"} {
		payload, _ := json.Marshal(shared.PacketLogFilter{Proto: proto})
		...
		if resp := d.Send(shared.Command{Type: shared.CmdGetPacketLog, Payload: payload}); !resp.Success ||
			...
			t.Fatalf("%s: %+v", proto, resp)
		}
		for _, e := range res.Entries {
			if e.ICMP == nil {
				t.Errorf("a demo %s row (%s from %s) has no ICMP type", proto, e.Rule, e.Src)
			} else if e.Rule == "icmp_flood" && e.ICMP.Type != map[string]uint8{"icmp": 8, "icmpv6": 128}[proto] {
...
**Interfaces:**
- Produces (shared): `type DropReason string`, thirteen `Drop*` constants, `var AllDropReasons []DropReason`, `type DropWhy struct { Code DropReason; Params map[string]any }`, `func (e PacketLogEntry) DropReason(r Rules, n NetworkSettings) DropWhy`, `var ICMPv4Accepted []uint8`, `func ICMPv6Accepted(IPv6Config) []uint8`.
- Produces (web): `var blockedRuleOption map[string]string`; template func `blockedOption`; `blockedRows.Rules`, `.Net`, `.WhyKnown`; locale keys `blocked_why_<code>` ×13 after `blocked_rule_other`, `blocked_detail_icmp` after `blocked_detail_mark`; CSS `.pkt-rule`, `.pkt-why`, `a.log-action`.
...
| Cost per `/blocked/rows` poll | +2 socket round trips (GET_RULES reads `rules.json`, `rules.go:108`; GET_APPLIED_CONFIG reads `applied-config.json`, `appliedconfig.go:30`), **only when a default-drop row is on screen**; 0 otherwise | today the poll makes 2 (`GetPacketLog`, `GetOptions`). Two small file reads on a local socket every 5 s is noise; skipping them for module-only views costs one `slices.ContainsFunc` |
...
// DropReason walks the input chain in nft.Apply's order — loopback, the IPv6
// mode, established, the ICMP accepts, the modules, the Docker networks, the
// blacklist, the whitelist, the port rules, the custom rules, the final log —
// and names the first step that would decide this packet differently now, or
// the step that was missing. r is the rule set the kernel holds (Current), n
// the network settings it was applied with.
//
// Steps it does not ask about: loopback and established cannot reach the final
// drop; a module that refuses logs under its own name; the custom rules are not
// parsed here (see Reachable); and the Docker networks are left out, because
// the auto-detected bridges cannot be listed from the web process.
func (e PacketLogEntry) DropReason(r Rules, n NetworkSettings) DropWhy {
	...
	for _, pr := range rules {
		if !PortInRule(pr.Port, e.DstPort) {
			continue
		}
		if !pr.FiltersHost() {
			forwarded = true
		...
		srcs := slices.DeleteFunc(slices.Clone(pr.Sources), IsListComment)
		switch {
		case len(pr.Sources) == 0:
...
Run `go test ./internal/core/ -run TestICMPAccepts` — PASS. Mutate: add `8` to `icmpv4Types` in `nftables.go:1159` → red; drop `136` from the NA append (`:1193`) → red; drop `12` from `shared.ICMPv4Accepted` → red. Restore.
...
```go
	...
	// holds (Current) and the network settings they were applied with. WhyKnown
	// is false when either could not be read, and then no row carries a reason
	// — none is better than one computed from a guess.
	Rules    shared.Rules
...
// and "now" means what the kernel holds.
func (s *Server) rulesNow(rows *blockedRows) {
	...
	rows.Rules, rows.Net, rows.WhyKnown = state.Current, applied.Config.Network, true
}
```
...
The keys are the toggles' `name=` attributes in `options.html` (`:43, 67, 91, 111, 127, 143, 159, 191`), which are the toml keys Task 6 anchors.
...
```html
  {{/* A module's chip leads to its card on /options (blockedOption); the
       ...
       holds now (shared.PacketLogEntry.DropReason). One .pkt-rule item, the
  ...
  <td data-label="{{T "blocked_col_rule"}}"><div class="pkt-rule">
    ...
    {{if $.WhyKnown}}{{with .DropReason $.Rules $.Net}}{{if .Code}}<span class="pkt-why">{{T (printf "blocked_why_%s" .Code) .Params}}</span>{{end}}{{end}}{{end}}
  </div></td>
```
...
`web/src/app.css`, after `.table tbody tr:focus-within .pkt-actions .btn { … }` (`:3321`):
...
Both files, directly after `blocked_rule_other` (`:633`):
...
And after `blocked_detail_mark` (`:613`): en `{"id": "blocked_detail_icmp", "translation": "ICMP type/code"},`, de `{"id": "blocked_detail_icmp", "translation": "ICMP-Typ/-Code"},`.
...
| `DropReason` (Task 8) walks it | Task 8 Step 2 | logic unchanged (meters log as `icmp_flood`/`tcp_rst`, never `drop`; accept lists unchanged, so `TestICMPAcceptsAreTheListsDropReasonReads` holds); one comment phrase stale — see *Coordination* |
...
grep -c 'Ping and reset rate limits' docs/assets/diagrams/rule-order-light.svg docs/assets/diagrams/rule-order-dark.svg
npm run check:prose
...
REPLACE = [
    ...
    ("`block`, IPv6 is decided before any module sees it.\n",
     "`block`, IPv6 is decided before any module sees it.\n"
     "\n"
     "Two modules come earlier than that too, ahead of return traffic: **ICMP flood** and\n"
...
- **Task 8**: `DropReason`'s doc comment lists the walk as "loopback, the IPv6 mode, established, the ICMP accepts, the modules …". After this task: "loopback, the IPv6 mode, the ping and reset meters, established, the ICMP accepts, the other modules …". Logic unaffected — the meters log under their own prefixes, never `drop`, and `addICMPRules` is untouched. `plan-P4.md:3-13`'s chain-order block goes stale the same way. Task 8's `blockedRuleOption` maps `fragment` → `drop_fragments`; still right.
