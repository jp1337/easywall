...
- D2: a host with **no IPv6 container network** gets a forward chain byte-identical to 2.23.2, with two named exceptions: under `ipv6.mode = "block"` a forwarded rule's IPv6 sources no longer render; and a container-network list with no usable network (only comments) no longer renders forwarded accepts with no deny.
- D3: under `ipv6.mode = "block"` no IPv6 container network reaches any consumer (input accepts, forward chain, published-port check, the record) — detected or `custom_networks` — and a forwarded rule's IPv6 sources do not render. `routing.networks` is not a container network and is untouched.
- D6 includes a bug found while planning: Docker's iptables backend writes DNAT as an xtables target (`*xt.NatRange2`), which the check never read (since 2.20.1). Fixed in Task 3, both families.
...
Expected: the second run PASSes (run it twice more: identical); the golden file is non-empty, contains `&expr.Cmp{Op:0x0, Register:0x1, Data:[]uint8{0x2}}` and **no** `0xc0…` address.
...
Run: `go test ./internal/core/ -run 'TestCidrFamilies|TestFamiliesOfAMappedNetworkIsIPv4|TestContainerNetworks|TestWithoutIPv6|TestForwardFamilyPin' -v`
...
```
   ...
   `dockerCIDRs` arrives already filtered for D3 by `containerNetworks`; `ipv6` is needed here only for a forwarded rule's own IPv6 sources. Keep the body as it is, with these edits: the early `return` becomes `return dockerCIDRs`; the call becomes `m.addForwardPortRules(t, c, rules, docker, dockerCIDRs, ipv6)`; the function ends with `return dockerCIDRs`. Update the diagram in its doc comment to the one in spec §2.
...
3. `ApplyWithFeeds` — replace the block that assembles `dockerCIDRs` (`var dockerCIDRs []string` … `dockerCIDRs = append(dockerCIDRs, docker.CustomNetworks...)` … `}`) with:
...
```
   ...
   and the forward-chain call with:
...
Run: `go test ./internal/core/ -run 'TestForwardChain_|TestCidrFamilies|TestFamiliesOf|TestContainerNetworks|TestWithoutIPv6|TestForwardFamilyPin' -v`
...
| Mutation | Must go red |
|---|---|
| remove the `withoutIPv6` call in `containerNetworks` | `TestContainerNetworks`, `…BlockKeepsIPv6AwayFromContainers` |
| remove the `fam == familyIPv6 && ipv6 == IPv6Block` skip | `…BlockKeepsIPv6AwayFromContainers` |
| build the IPv6 copy from a rule with `ID` cleared | `…TheTwinCarriesTheRulesID` |
...
| `cidrFamilies`/`withoutIPv6` parse with `netip.ParsePrefix` instead of `shared.ParseNetwork` (the mapped prefix stays `Is4In6`) | `TestFamiliesOfAMappedNetworkIsIPv4` |
| `cidrFamilies` treats an unparseable entry as IPv4 | `…AListWithNoUsableNetworkRendersNoAccept` |
| emit an IPv6 copy even when `v6` is false | `…AnIPv4OnlyHostIsByteIdentical` |

- [ ] **Step 8: Commit**

```bash
git add internal/core/forward_family.go internal/core/forward_family_test.go internal/core/nftables.go internal/core/nftables_forward_scope_test.go internal/core/forward_golden_test.go
...
### Task 3: The published-port check reads `ip6` — and Docker's iptables backend
...
- Produces: `publishedPortFromRule(r *nftables.Rule, nets []netip.Prefix, family nftables.TableFamily) (publishedPort, bool)`; `publishedPort.addr` is `"::"` for an IPv6 rule with no `ip6 daddr`; `func xtDNATTarget(info xt.InfoAny) (netip.Addr, uint16, bool)`.

**A bug this task also fixes (spec D6).** `iptables-nft`/`ip6tables-nft` 1.8.11 — what Docker's default iptables backend runs — write `-j DNAT` as an xtables target, not an nft `nat` expression. The decoder reads only `expr.NAT`, so since 2.20.1 the check has named nothing on such hosts; its fixture is written with `nft -f`, which produces the other encoding, and passed. Both encodings below were dumped from a live kernel on 2026-09-26 and are the only evidence the decoder is written against.

nft (`unshare -rn nft -d netlink -f …`):
...
iptables-nft, read back through google/nftables (the library decodes the target's info):

```
ip nat DOCKER    ip daddr 172.17.0.1 udp dport 53 → 172.18.0.2:53
  Payload{Base:1 Offset:16 Len:4} Cmp{[172 17 0 1]} Payload{Base:1 Offset:9 Len:1} Cmp{[17]}
  Payload{Base:2 Offset:2 Len:2} Cmp{[0 53]} Counter
  Target{Name:DNAT Rev:2 Info:*xt.NatRange2{Flags:3 MinIP:172.18.0.2 MaxIP:172.18.0.2 MinPort:53 MaxPort:53}}
ip nat DOCKER    tcp dport 7070 → 172.18.0.9 (no port)
  … Target{Name:DNAT Rev:2 Info:*xt.NatRange2{Flags:1 MinIP:172.18.0.9 MinPort:0}}
ip6 nat DOCKER   ip6 daddr 2001:db8::1 tcp dport 25 → [fd00:ea5e::4]:2525
  Payload{Base:1 Offset:24 Len:16} Cmp{2001:db8::1} Meta{Key:l4proto} Cmp{[6]}
  Payload{Base:2 Offset:2 Len:2} Cmp{[0 25]} Counter
  Target{Name:DNAT Rev:2 Info:*xt.NatRange2{Flags:3 MinIP:fd00:ea5e::4 MinPort:2525}}
```

`Flags` bit 1 (`xt.NatRangeProtoSpecified`) says whether `MinPort` translates the port. `ip6tables-nft` uses `meta l4proto` for `-p`, like nft — no next-header payload case is needed.

- [ ] **Step 0: Unit tests against both encodings**

Append to `internal/core/docker_published_ports_test.go` (imports: `net`, `net/netip`, `github.com/google/nftables`, `github.com/google/nftables/expr`, `github.com/google/nftables/xt`):
...
Run: `go test ./internal/core/ -run TestPublishedPortFromRuleReadsBothEncodings -v` → FAIL (compile: the function takes two arguments).
...
Run (root, as CI): `sudo env PATH="$PATH" EASYWALL_REQUIRE_SELFTEST=1 go test -tags integration -count=1 -run 'TestIntegration_IPv6PublishedPortsAreRead|TestIntegration_DockersIptablesRulesAreReadAndNamed' ./internal/core/ -v`
Expected: FAIL — nothing read in either (the second also fails for IPv4: the xtables encoding is the bug).
...
Replace `publishedPortFromRule` with the version below (import `github.com/google/nftables/xt`); the IPv4 payload cases are unchanged, the ip6 daddr case and the xtables target are new:
...
Rewrite the doc comments of `detectPublishedPorts` ("IPv4 only…") and `publishedPortFromRule` (the example rule, "Deliberately forgiving…") to describe both families and both encodings, citing the dumps above; keep the paragraph about the forgiving direction.

- [ ] **Step 3: Run**

Run: `sudo env PATH="$PATH" EASYWALL_REQUIRE_SELFTEST=1 go test -tags integration -count=1 -run 'TestIntegration_.*PublishedPort|TestIntegration_NoBridgeMeansNoPublishedPorts|TestIntegration_DockersIptablesRules' ./internal/core/ -v` and `go test ./internal/core/ -run 'Published' -v`
...
| Mutation | Must go red |
|---|---|
| list only `TableFamilyIPv4` again | `TestIntegration_IPv6PublishedPortsAreRead`, `…DockersIptablesRulesAreReadAndNamed` |
| delete the offset-24 case | `TestPublishedPortFromRuleReadsBothEncodings` (both ip6 cases) |
| default `p.addr` stays `"0.0.0.0"` for ip6 | `…IPv6PublishedPortsAreRead` (the `::` keys) |
| delete the `case *expr.Target` branch | `TestPublishedPortFromRuleReadsBothEncodings` (xt cases), `…DockersIptablesRulesAreReadAndNamed` |
| ignore `NatRangeProtoSpecified` (always take `MinPort`) | `…ReadsBothEncodings` ("no port translates the address only") |
...
git commit -m "fix(core): the published-port check reads Docker's iptables-nft rules and the ip6 family (2.24 D6)

iptables-nft writes -j DNAT as an xtables target; the check read only nft's
nat expression, so since 2.20.1 it named nothing on hosts using Docker's
...
In `internal/web/democlient.go`: add a field `unknownBridge bool` to `demoState`, set it `true` at the end of `seed()` (`democlient.go:159`), set it `false` in the demo's apply handler `handleApplyRules` (`democlient.go:815`), after its refusal checks — only an apply that goes through names the network, and in `statusLocked`:
...
```html
    ...
    <div role="status" class="alert alert-warn mb-4">
      ...
      <span>{{T "dashboard_unknown_bridge" (dict "Iface" .Interface "CIDRs" .CIDRList)}} <a class="link" href="/apply">{{T "nav_apply"}}</a></span>
    </div>
    {{end}}{{end}}
```
...
| file | text |
|---|---|
| `en` | `Docker network {{.Iface}} ({{.CIDRs}}) exists and is not in the rules in force. Apply to include it.` |
| `de` | `Das Docker-Netz {{.Iface}} ({{.CIDRs}}) existiert und steht nicht in den aktiven Regeln. Anwenden nimmt es auf.` |
| `fr` | `Le réseau Docker {{.Iface}} ({{.CIDRs}}) existe et ne figure pas dans les règles en vigueur. Appliquez-les pour l'inclure.` |

Neutral on purpose (spec D5): under `routing.mode = "open"` an unknown network is less closed, not more, so no consequence is printed. The link label is the sidebar's `nav_apply` (*Apply Rules* / *Regeln anwenden* / *Appliquer les règles*), a standalone label — not `link_apply_rules`, which is a fragment for the `staged_note` slot.
...
**Interfaces:**
- Consumes: `NewNftablesManager` via `newIntegrationManager`, `tcpReaches` (`reach_integration_test.go`), `skipOrFailUnprovable`, `router.holdNamespace/run/inNS` (`nftables_forward_test.go`), `easywallInetTable`, `ruleFamily` (Task 2), `NftablesManager.BakedBridges()` (Task 4 — Task 6 starts after Task 4, ledger ruling P1).
...
func newRouter6(t *testing.T) *router6 {
	...
	mac := func(pid, dev string) string {
		args := []string{"cat", "/sys/class/net/" + dev + "/address"}
		if pid != "" {
...
- [ ] **Step 5: Release files** — `CHANGELOG.md` `## [2.24.0] — <date>` with the tagline **"Containers answer over IPv6."**, *Added* (IPv6 container forwarding, unknown-bridge line, ip6 published-port check), *Changed* (`ipv6.mode = "block"` also keeps IPv6 from containers; under `block` a forwarded rule's IPv6 sources no longer render; a container-network list with no usable network no longer renders forwarded accepts; containers on an IPv6 bridge reach the host's services over IPv6 as over IPv4), *Fixed* (**the published-port warning named nothing on hosts where Docker uses its default iptables backend, since 2.20.1** — iptables-nft writes DNAT as an xtables target the check did not read), and the link definitions; `debian/changelog` entry; `CurrentVersion = "2.24.0"`; `docs/_config.yml` `version: "2.24.0"`.
